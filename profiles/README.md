...
- They expected the gamma 2.2 values Nikon Scan's driver hands its CMS. The
  ...
  samples the scanner returns. Checked with littlecms: a linear half comes back
  at Y 0.49 to 0.51 for all of them.

Nothing else was touched. The measurements are Nikon's.

## What is here

`_P` positive, `_N` negative, `_K` Kodachrome, `_MN` monochrome negative. The
LS-8000 and LS-4000 have no `_MN`.
...
Copyright Nikon Inc. and Nikon Corporation 2003, from the Nikon Scan 4
installer. Not covered by this crate's MIT or Apache-2.0 license.
