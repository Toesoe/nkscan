...
typedef BOOL(WINAPI *pDeviceIoControl)(HANDLE hDevice, DWORD dwIoControlCode, LPVOID lpInBuffer, DWORD nInBufferSize,
...
static void log_write(const char *fmt, ...)
{
    ...
    int prefix = snprintf(buf, sizeof(buf), "[%02d:%02d:%02d.%03d +%10.3fms] ", st.wHour, st.wMinute, st.wSecond,
                          st.wMilliseconds, ms);
...
static void dump_hex(const unsigned char *data, unsigned int len)
{
    unsigned int preview = len < 64 ? len : 64;

    log_write("    First %u bytes:\n      ", preview);

    for (unsigned int i = 0; i < preview; i++)
    {
        log_printf("%02X ", data[i]);

        if ((i & 15) == 15 && i + 1 < preview) log_printf("\n      ");
    }
    ...
    if (len > 64)
    {
        log_write("    Last 64 bytes:\n      ");

        for (unsigned int i = len - 64; i < len; i++)
        {
            log_printf("%02X ", data[i]);

            if (((i - (len - 64)) & 15) == 15 && i + 1 < len) log_printf("\n      ");
        }
...
static void dump_sense(const unsigned char *sense, int len)
{
    ...
    for (int i = 0; i < len; i++) log_printf("%02X ", sense[i]);
    ...
    if (asc == 0x3A && ascq == 0x00) log_write("  -> NO MEDIUM\n");

    if (asc == 0x28 && ascq == 0x00) log_write("  -> MEDIUM CHANGED\n");
}

static void decode_nikon_vendor(const unsigned char *cdb, int len, int write, const unsigned char *data,
                                unsigned int data_len)
{
    ...
    switch (reg)
    {
        ...
        default:
            log_write("    0x%02X: UNKNOWN\n", reg);
            break;
    }

    if (len >= 10)
    {
        unsigned int transfer = ((unsigned int)cdb[6] << 16) | ((unsigned int)cdb[7] << 8) | (unsigned int)cdb[8];
    ...
    if (data && data_len)
    {
        log_write("    %s payload (%u bytes): ", write ? "WRITE" : "READ", data_len);
...
static void bin_write_scsi(DWORD seq, const SCSISCAN_CMD *cmd, BOOL result, const BYTE *data, DWORD dataLen,
                           BYTE srbStatus, const BYTE *sense, DWORD senseLen)
{
    if (g_binFile == INVALID_HANDLE_VALUE) return;
    DWORD w;
    BYTE  dir     = (cmd->SrbFlags & SRB_FLAGS_DATA_IN) ? 1 : (cmd->SrbFlags & SRB_FLAGS_DATA_OUT) ? 2 : 0;
...
static BOOL WINAPI hookedDeviceIoControl(HANDLE hDevice, DWORD dwIoControlCode, LPVOID lpInBuffer, DWORD nInBufferSize,
                                         LPVOID lpOutBuffer, DWORD nOutBufferSize, LPDWORD lpBytesReturned,
                                         LPOVERLAPPED lpOverlapped)
{
    ...
    if (dwIoControlCode == IOCTL_SCSISCAN_CMD && lpInBuffer && nInBufferSize >= sizeof(SCSISCAN_CMD))
    {
        ...
        log_write("  CdbLength=%d SrbFlags=0x%08X (dir:%s) TransferLen=%d "
                  ...
                  cmd->CdbLength, cmd->SrbFlags, isDataIn ? "IN" : (isDataOut ? "OUT" : "none"), cmd->TransferLength,
        ...
        result = realDeviceIoControl(hDevice, dwIoControlCode, lpInBuffer, nInBufferSize, lpOutBuffer, nOutBufferSize,
        ...
        if (cmd->pSenseBuffer && cmd->SenseLength > 0 && !IsBadReadPtr(cmd->pSenseBuffer, cmd->SenseLength))
        {
         ...
         * By now the DMA has completed (we polled for it above). */
        {
            ...
            DWORD       dataLen = 0;
            if ((isDataIn || isDataOut) && outLen > 0 && lpOutBuffer && !IsBadReadPtr(lpOutBuffer, outLen))
            {
                dataPtr = (const BYTE *)lpOutBuffer;
                dataLen = outLen;
            }
            bin_write_scsi(seq, cmd, result, dataPtr, dataLen, srbStatus, sensePtr, senseLen);
        }
...
HANDLE WINAPI hookedCreateFileA(LPCSTR name, DWORD access, DWORD share, LPSECURITY_ATTRIBUTES sec, DWORD creation,
                                DWORD flags, HANDLE template)
{
    log_write("CreateFileA(%s)\r\n", name);
    HANDLE h = realCreateFileA(name, access, share, sec, creation, flags, template);
...
HANDLE WINAPI hookedCreateFileW(LPCWSTR name, DWORD access, DWORD share, LPSECURITY_ATTRIBUTES sec, DWORD creation,
                                DWORD flags, HANDLE template)
{
    HANDLE h = realCreateFileW(name, access, share, sec, creation, flags, template);
...
BOOL WINAPI hookedWriteFile(HANDLE h, LPCVOID buffer, DWORD len, LPDWORD written, LPOVERLAPPED ov)
{
    if (h == g_scannerHandle) log_hex("USB OUT", buffer, len);

    return realWriteFile(h, buffer, len, written, ov);
}

BOOL WINAPI hookedReadFile(HANDLE h, LPVOID buffer, DWORD len, LPDWORD read, LPOVERLAPPED ov)
{
    BOOL r = realReadFile(h, buffer, len, read, ov);

    if (h == g_scannerHandle && r) log_hex("USB IN", buffer, *read);

    return r;
}

/* --- IAT hook installation --- */
static void hookIat(HMODULE mod, const char *dll, const char *func, FARPROC replacement, FARPROC *original)
{
    ...
    for (; imp->Name; imp++)
    {
        ...
        if (lstrcmpiA(name, dll)) continue;
        ...
        {
            IMAGE_IMPORT_BY_NAME *entry = (IMAGE_IMPORT_BY_NAME *)(base + orig->u1.AddressOfData);

            if (strcmp((char *)entry->Name, func) != 0) continue;
...
__declspec(dllexport) int WINAPI NkDriverEntry(DWORD op, DWORD param2, DWORD param3)
{
    ...
    if (op == 5)
    {
        ...
        fake.SrbFlags       = p->direction == 1 ? SRB_FLAGS_DATA_IN : p->direction == 2 ? SRB_FLAGS_DATA_OUT : 0;
        ...
        if (p->direction == 1)
        {
            ...
            bin_write_scsi(seq, &fake, result, (BYTE *)p->data_buffer, p->transfer_length, 0, NULL, 0);
...
BOOL WINAPI DllMain(HINSTANCE hinstDLL, DWORD fdwReason, LPVOID lpvReserved)
{
    ...
    switch (fdwReason)
    {
        ...
        {
            g_logFile = CreateFileA("C:\\usb_trace.log", GENERIC_WRITE, FILE_SHARE_READ, NULL, CREATE_ALWAYS,
                                    FILE_ATTRIBUTE_NORMAL, NULL);
            g_binFile = CreateFileA("C:\\usb_trace.bin", GENERIC_WRITE, FILE_SHARE_READ, NULL, CREATE_ALWAYS,
                                    FILE_ATTRIBUTE_NORMAL, NULL);
            ...
            if (!g_realDll) g_realDll = LoadLibraryA("NKDUSCAN_real.dll");
            if (!g_realDll)
            {
            ...
            if (!g_realNkDriverEntry) g_realNkDriverEntry = (pNkDriverEntry)GetProcAddress(g_realDll, (LPCSTR)1);
            if (!g_realNkDriverEntry)
            {
            ...
            hookIat(g_realDll, "kernel32.dll", "CreateFileA", (FARPROC)hookedCreateFileA, (FARPROC *)&realCreateFileA);

            hookIat(g_realDll, "kernel32.dll", "CreateFileW", (FARPROC)hookedCreateFileW, (FARPROC *)&realCreateFileW);

            hookIat(g_realDll, "kernel32.dll", "ReadFile", (FARPROC)hookedReadFile, (FARPROC *)&realReadFile);

            hookIat(g_realDll, "kernel32.dll", "WriteFile", (FARPROC)hookedWriteFile, (FARPROC *)&realWriteFile);

            hookIat(g_realDll, "kernel32.dll", "DeviceIoControl", (FARPROC)hookedDeviceIoControl,
                    (FARPROC *)&realDeviceIoControl);
