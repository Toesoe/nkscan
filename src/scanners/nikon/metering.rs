//! Host-side exposure metering
//!
//! A low-resolution pass is decoded here and the per-channel gain scaled so the film's high
//! tail lands near the top of the range. Gain is linear in the value, so one proportional step
//! lands it and a second pass measures from where the first got to.
//!
//! Shared because both the LS-9000 and the LS-5000 meter this way: the scanner runs an ordinary
//! pass and the host decides what the gains become. Only the pass geometry and the defaults are
//! per model.

use super::{Channel, ChannelExposures};
