//! Reading page 0xC1 on the USB bodies
//!
//! 4000 DPI optical, boundaries of 3946 by 5959 dots, a 5959-dot frame pitch and focus 0 to 323.
//! Offset 71 counts roll-feeder slots.
//!
//! The LS-50 and the LS-5000 answer this identically apart from the converter width, so the one
//! byte they disagree about is a parameter here rather than a second copy of the file.
...
pub fn read<T: Transport + ?Sized>(transport: &mut T) -> Result<DeviceLimits, scsi::Error> {
...
pub fn read_sensed_frames<T: Transport + ?Sized>(transport: &mut T) -> u32 {
    transport
...
pub mod fixture {
    ...
    pub fn capabilities(max_bits: u8) -> DeviceLimits {
        DeviceLimits::from_page(&page(max_bits, 0)).expect("the fixture page parses")
    }

    /// The whole response, header included, as a mock transport has to answer it
    pub fn raw_page(max_bits: u8) -> Vec<u8> {
        let body = page(max_bits, 0).data;
    ...
    /// The page a unit answers, with `slots` roll positions sensed
    ///
    ...
    pub fn page(max_bits: u8, slots: u8) -> VpdPage {
...
mod tests {
    ...
    fn the_allocation_length_covers_the_page() {
        assert!(usize::from(ALLOCATION_LENGTH) >= fixture::page(14, 0).data.len());
    }

    #[test]
    fn parses_the_captured_page() {
        let caps = DeviceLimits::from_page(&fixture::page(14, 0)).unwrap();
    ...
    fn the_sample_depth_is_fourteen_bits() {
        assert_eq!(
            DeviceLimits::from_page(&fixture::page(14, 0))
                .unwrap()
                .max_bits,
            14
        );
    ...
    fn reads_the_sensed_slot_count() {
        assert_eq!(frames_in(&fixture::page(14, 0)), Some(0));
        assert_eq!(frames_in(&fixture::page(14, 40)), Some(40));
    }
    ...
    fn loading_a_roll_does_not_change_the_geometry() {
        let empty = DeviceLimits::from_page(&fixture::page(14, 0)).unwrap();
        let loaded = DeviceLimits::from_page(&fixture::page(14, 40)).unwrap();
    ...
    fn a_zero_boundary_is_not_a_usable_page() {
        for offset in [32, 54] {
            let mut page = fixture::page(14, 0);
    ...
    fn rejects_another_page() {
        let mut other = fixture::page(14, 0);
