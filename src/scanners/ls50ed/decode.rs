...
//!
//! Planar, line by line: one plane per channel back to back, `R G B` or `R G B I`, top line
...
impl FrameDecoder {
    pub fn new(settings: &ScanSettings) -> Self {
        Self::wrap(PlanarLines::new(settings))
    }
}

impl PlanarLines {
    fn new(settings: &ScanSettings) -> Self {
    ...
    /// `new` derives these from the scan geometry; tests set them directly
    fn from_dims(width: usize, height: usize, n_colors: usize, stride: usize) -> Self {
        Self {
            width,
            height,
            n_colors,
...
mod tests {
    ...
    fn decode(data: &[u8], width: usize, height: usize, n_colors: usize, stride: usize) -> Image {
        let mut decoder = Blocked::wrap(PlanarLines::from_dims(width, height, n_colors, stride));
    ...
    fn reassembles_a_line_split_across_pushes() {
        let mut decoder = Blocked::wrap(PlanarLines::from_dims(2, 1, 3, 12));
    ...
    fn short_stream_is_an_error() {
        let mut decoder = Blocked::wrap(PlanarLines::from_dims(2, 2, 3, 12));
